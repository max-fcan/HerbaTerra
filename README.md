...
| --- | --- |
| App stays on the loading page | Verify the Turso URL and token in `.env.production`, then inspect `GET /api/db/replica-status` |
| `libsql` import errors | Reinstall dependencies from `requirements.txt` inside `.venv` |
| Map data does not render | Confirm the GeoJSON files exist in `data/` and the configured resolution is valid |
| Port already in use | Change `PORT` in `.env.production` |
| Local replica path is wrong | Set `LOCAL_DB_PATH` explicitly and keep it under `temp/` |
